...
/*   Updated: 2023/05/03 14:45:35 by pfaria-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	wait_pids_bonus(pid_t *pid, t_shell *shell, int end, int i)
{
	...
	while (i <= end)
	{
		if (shell->pid[i] != -1)
		{
			printf("Je wait pid le pid numero : %d\n", i);
	...
	struct stat buff)
{
	...
	if (shell->index_of_pipes != shell->nb_of_pipes && shell->current_cmdb == 0)
	{
		printf("je pipe %s le fd que je pipe est = %d\n", command[0], shell->index_of_pipes);
	...
	{
		if (shell->current_cmdb == 0)
			redirection_bonus(shell);
		fprintf(stderr, "j'execute %s\n", command[0]);
