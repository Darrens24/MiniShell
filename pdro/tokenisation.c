...
/*   Updated: 2023/01/30 13:09:33 by pfaria-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*envfinder(char *line, char *newvar, t_chained *env)
{
	...
	while (elem)
	{
		if (ft_strncmp(line, elem->variable, len) == 0)
			newvar = ft_strjoin(newvar, ft_strndup(line, len+1, ft_strlen(elem->variable) - 1));
		elem = elem->prev;
	}
	return (newvar);
}
	...
	while (token->var[i] && token->var[i] != '\"')
	{
		...
		if (token->var[i] == '$')
		{
			...
			if (token->var[i] == '{')
			{
				...
				newvar = envfinder(token->var, newvar, env);
			}
		}
		else
